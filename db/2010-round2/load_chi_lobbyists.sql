...
SELECT DISTINCT lower(replace(replace(replace(l.first_name, ',', ''), ' ', '-'), '.', '')) || '-' || lower(replace(replace(replace(replace(l.last_name, ',', ''), '''', '-'), ' ', '-'), '.', '')), l.last_name, l.first_name, l.full_name, t.termination_date
